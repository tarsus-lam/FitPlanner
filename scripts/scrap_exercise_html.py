...
# Function to extract the image and description from HTML content
def extract_data(html_content, name):
    soup = BeautifulSoup(html_content, 'html.parser')
    # Find the section containing the image and description based on the <h3> heading
    section_heading = soup.find('h3', class_ = 'ExHeading ExHeading--h3', string = lambda text: text and 'Instructions' in text)
    
    if section_heading:
        # Extract the image and description from the section
        ...
        print(f"'Instructions' section heading not found for '{name}'.")
        return None
...
def write_csv_header(csv_writer):
    csv_writer.writerow(['Name', 'Muscle', 'Equipment', 'Rating', 'Description'])
...
def aggregate_exercise_data(soup, outdir, start_name = None):
    ...
    with open_csv_file(start_name, outdir) as csv_file:
        ...
        for exercise_row in rows_to_process:
            ...
            if html_content:
                description = extract_data(html_content, name)
            else:
                description = None
...
def parse_arguments():
    ...
    parser.add_argument('--html_text', type=str, required = True, help = 'HTML text of exercises to be processed')
    parser.add_argument('--prev_exercise', type = str, help = 'Last exercise extracted before termination', default = None)
    parser.add_argument('--output_dir', type = str, required = True, help = 'Directory to save CSV file')
